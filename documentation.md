...
This file tells you (Claude) what to do in this Phoenix project. Work through the
...
1. A `README.md` that gets a brand-new developer from clone → running server,
   **and** a comprehensive set of docs that actually explain the system — its
   ...
   alone is not enough; document what the project *is* and how it fits together.
...
Adapt the checklist to match the project's real tooling — if it uses `mix credo`,
`mix dialyzer`, or a CI step, add those lines.
